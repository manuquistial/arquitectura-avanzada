...
async def get_database_info() -> dict:
    ...
    try:
        ...
        logger.error(f"❌ Database connection error getting info: {e}")
        return {
            "status": "connection_error",
            "error": str(e)
        }
    except TimeoutError as e:
        logger.error(f"❌ Database timeout error getting info: {e}")
        return {
...
async def init_db() -> None:
    ...
    try:
        ...
        if not await test_connection():
            ...
            logger.warning("⚠️  Continuing without database connection test")
        else:
        ...
        logger.warning("⚠️  Database initialization failed due to connection error, but continuing...")
    except TimeoutError as e:
        logger.error(f"❌ Database timeout error during initialization: {e}")
        logger.warning("⚠️  Database initialization failed due to timeout, but continuing...")
    except Exception as e:
